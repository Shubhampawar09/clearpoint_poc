...
describe("Validate create measures", () => {
  ...
  it("Verify and create add measure", () => {
    ...
    utils.clickOn(addMeasures.ownerDropdown);
  ...
  it("Verify the measure is present or not", () => {
    cy.visit(pageUrl);
    var element =addMeasures.verifyMeasure(title)
    utils.scrollToElement(element).should("be.visible", true);
  });
  ...
  it("verify add and delete duplicate measures", () => {
    ...
    utils
      ...
      .contains("Measure Deleted").should('be.visible');
  });
});
